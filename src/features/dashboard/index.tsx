    ...
    <>
      ...
      <Main className='space-y-4 bg-background text-foreground'>
        ...
        <section className='rounded-xl border border-border bg-card/80 p-4 backdrop-blur dark:border-slate-700/80 dark:bg-slate-900/70'>
          <div className='relative h-[390px] overflow-hidden rounded-2xl border border-cyan-300/25 bg-slate-950'>
            <div className='absolute inset-0 bg-gradient-to-r from-slate-950 via-sky-950 to-slate-900' />
            <div className='absolute inset-0 bg-[linear-gradient(rgba(125,211,252,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(125,211,252,0.08)_1px,transparent_1px)] bg-[size:40px_40px] opacity-30' />
            <div className='absolute left-0 right-0 bottom-0 h-44 bg-gradient-to-t from-cyan-950/40 to-transparent' />

            <div className='absolute inset-x-0 bottom-10 flex items-end justify-center gap-8 px-8'>
              {[120, 170, 145, 195, 160, 210].map((height, index) => (
                <div
                  key={`tower-${index}`}
                  className='w-12 rounded-t-lg border border-cyan-200/10 bg-gradient-to-t from-cyan-700/30 via-sky-300/20 to-slate-100/30 shadow-[0_0_24px_rgba(34,211,238,0.25)]'
                  style={{ height }}
                />
              ))}
            </div>

            <div className='absolute left-6 top-6 z-10 rounded-2xl border border-cyan-200/20 bg-slate-950/60 px-6 py-5 backdrop-blur'>
              <p className='text-sm text-slate-300'>Active fault map · Warning/Critical only</p>
              <p className='mt-2 text-3xl font-semibold uppercase tracking-[0.14em] text-slate-100'>Network Topology</p>
              <p className='mt-2 text-xl text-slate-300'>Smart fault corridors and anomaly radar</p>
            </div>
          </div>
        </section>

            <div className='absolute right-8 top-8 rounded-2xl border border-cyan-200/20 bg-slate-950/60 px-5 py-4 text-xs text-slate-300 backdrop-blur'>
              <p className='text-sm uppercase tracking-[0.22em] text-cyan-200'>Telemetry</p>
              <p className='mt-2 font-mono text-lg text-cyan-100'>Sites 410 · Nodes 92 · Cells 2,840</p>
            </div>
            ...
            {markers.map((kpi) => (
              <div
                ...
                style={{ left: `${kpi.marker?.x}%`, top: `${kpi.marker?.y}%` }}
              >
                <div className={`absolute inset-0 animate-ping rounded-full ${kpi.status === 'Critical' ? 'bg-rose-500/40' : 'bg-amber-400/45'}`} />
                ...
                <div className='pointer-events-none absolute left-1/2 top-full z-20 mt-2 hidden w-64 -translate-x-1/2 rounded-lg border border-slate-700 bg-slate-950/95 p-3 text-xs text-slate-200 shadow-lg group-hover:block'>
                  ...
                  <p className='mt-1'>
                    Status:{' '}
                    <span className={kpi.status === 'Critical' ? 'text-rose-300' : 'text-amber-300'}>{kpi.status}</span>
                  </p>
            ...
            <ul className='absolute bottom-7 left-7 flex list-none items-center gap-7 rounded-2xl border border-cyan-300/20 bg-slate-950/75 px-6 py-3 text-sm text-slate-200 backdrop-blur'>
