...
def restore_unlinked_dynamic_pages(conn: sqlite3.Connection) -> None:
    def ensure_sidebar_item(group_title: str, title: str, parent_id: str | None, sort_order: int, url: str | None = None) -> str:
        row = conn.execute(
            '''
            SELECT id
            FROM sidebar_items
    ...
    for row in rows:
        ...
        for index, part in enumerate(parts):
            ...
            parent_id = ensure_sidebar_item('General', part, parent_id, (index + 1) * 10, leaf_url)
...
def build_sidebar_tree(rows: list[sqlite3.Row]) -> list[dict]:
    ...
    def cleanup(node: dict, lineage: set[str] | None = None) -> dict:
        lineage = set() if lineage is None else set(lineage)
        node_id = node['id']
        if node_id in lineage:
            # Break corrupted cycles (A->B->A) to avoid recursion crash/500.
            return {
                'id': node['id'],
                'title': node['title'],
                'displayMode': node['displayMode'],
                **({'url': node['url']} if 'url' in node else {}),
                **({'badge': node['badge']} if 'badge' in node else {}),
            }
        lineage.add(node_id)
        ...
        if children:
            out['items'] = [cleanup(child, lineage) for child in children]
            if 'badge' in node:
